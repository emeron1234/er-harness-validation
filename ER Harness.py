...
# spark.sql('DROP TABLE IF EXISTS altrata_data_lake_poc_dev.synthetic_er_harness.er_test_report_v2')
# validation_table = f"altrata_data_lake_poc_dev.synthetic_er_harness.er_test_report_v2"
# # spark.sql(f'DROP TABLE IF EXISTS {validation_table}')
# final_df.write.mode("append").saveAsTable(f'{validation_table}')
# final_df.display()
...
# MAGIC %md
# MAGIC ### ER Harness Tables

# COMMAND ----------

# %sql
# select count(*) from altrata_data_lake_poc_dev.synthetic_er_harness.er_test_pam_matching_pairs_sorted_by_overall_score

# COMMAND ----------

# source_dataframe = spark.sql(f"""SELECT {selected_columns} FROM altrata_data_lake_poc_dev.synthetic_er_harness.er_test_pam_matching_pairs_sorted_by_overall_score limit 10""").display()

# COMMAND ----------

# MAGIC %sql
# MAGIC -- select * from altrata_data_lake_poc_dev.er_harness.er_test_truth

# COMMAND ----------

# %sql
# select * from altrata_data_lake_poc_dev.er_harness.er_test_report
# where batch_id = '20241209091018'
